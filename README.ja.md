...
### 1. 非 ASCII バイトがデフォルトフォントの描画を壊す

stock ファームウェアは TFT_eSPI の既定 5×7 GFX ビットマップフォントを使っており、
ASCII のみ対応。`0x80`–`0xFF` のバイト（UTF-8 継続バイトと emoji 先頭バイト全て）
はグリフテーブル範囲外をインデックスし、実測ではハートビート書き込みから ~1 秒以内に
radio タスクがハードリセットされるコードパスが十分あります。

**誰も有効化していない隠れた CJK 経路があります**：`M5StickCPlus` ライブラリには
1.7 MB の HZK16 GB2312 フォントが同梱されていて、`M5Display::loadHzk16(InternalHzk16)`
という API で起こせます。これを呼べば簡体字（~7000 GB2312 文字、16×16 px）が描画可能に
なります。ただし対応は **簡体字のみ** で、繁体字・日本語のかな・ハングルは含まれません。
stock ファームウェアはこの API を一度も呼ばず、加えてブリッジ側で UTF-8 → GBK 変換を
入れないとバイト対が GB2312 ルックアップに当たりません。

**回避：** `protocol.py` の `sanitize_for_stick()` が送信前に `0x20`–`0x7E`
（および tab）以外のすべてを `?` に書き換えます。可逆性は失われますが安定します。
